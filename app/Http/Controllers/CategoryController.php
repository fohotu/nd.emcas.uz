...
class CategoryController extends Controller
{
    ...
    */
    ...
    public function liveSearch_1(CategoryService $service, Request $request)
    {    
        $query = $request->only(['title','description']);
        $category = $service->getAllCategory($query);
    ...
    /*
    public function liveSearch(CategoryService $service, Request $request)
    {
        $query = $request->only(['title', 'description']);

        $categories = $service->getAllCategory($query);
    ...
    */
     ...
     public function liveSearch(CategoryService $service, Request $request)
    {    
        $query = $request->only(['title','menu']);
        $categories = $service->liveSearch($query);
        return response()->json($categories);
    }

    public function getCategoryByMenu(Request $request, CategoryService $service)
    {
        $menuId = $request->input('menu_id');
        $categories = $service->getCategoryByMenu($menuId);
